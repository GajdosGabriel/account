...
<template>
    ...
    <div class="space-y-7">
        ...
        <div class="relative overflow-hidden rounded-2xl bg-gradient-to-br from-brand-600 via-brand-600 to-violet-700 p-6 text-white shadow-lg shadow-brand-600/20 sm:p-8">
            ...
            <div class="relative">
                <p class="text-xs font-medium uppercase tracking-wider text-white/60">{{ t('dashboard.summary.mrr') }}</p>
            ...
            <div class="relative mt-7 grid grid-cols-2 gap-3 sm:grid-cols-4">
                <div class="rounded-xl bg-white/10 px-4 py-3 backdrop-blur">
                    ...
                    <p class="text-xs text-white/60">{{ t('dashboard.summary.organizations') }}</p>
                </div>
                <div class="rounded-xl bg-white/10 px-4 py-3 backdrop-blur">
                    <p class="text-2xl font-semibold">{{ stats.active }}</p>
                    <p class="text-xs text-white/60">{{ t('dashboard.summary.active') }}</p>
                </div>
                <div class="rounded-xl bg-white/10 px-4 py-3 backdrop-blur">
                    <p class="text-2xl font-semibold">{{ stats.trialing }}</p>
                    <p class="text-xs text-white/60">{{ t('dashboard.summary.trialing') }}</p>
                </div>
                <div class="rounded-xl bg-white/10 px-4 py-3 backdrop-blur">
                    <p class="text-2xl font-semibold">{{ stats.past_due + stats.suspended }}</p>
                    <p class="text-xs text-white/60">{{ t('dashboard.summary.payment_issues') }}</p>
                </div>
            </div>
        </div>
        ...
        <div>
            <div class="mb-4 flex items-baseline justify-between gap-3">
                <h2 class="text-sm font-semibold uppercase tracking-wide text-slate-500">
                    {{ t('dashboard.invoicing.heading') }}
                </h2>
                <Link href="/invoices" class="text-sm text-brand-700 hover:underline">
                    {{ t('dashboard.invoicing.all') }}
                </Link>
            </div>

            <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                <div class="card p-4">
                    <p class="text-xs text-slate-500">{{ t('dashboard.invoicing.invoiced_month') }}</p>
                    <p class="mt-1 text-2xl font-semibold tracking-tight text-slate-900">
                        {{ invoicing.invoiced_month?.formatted }}
                    </p>
                    <p class="mt-0.5 text-xs text-slate-400">
                        {{ tc('dashboard.invoicing.documents', invoicing.invoiced_month?.count ?? 0) }}
                    </p>
                </div>

                <div class="card p-4">
                    <p class="text-xs text-slate-500">{{ t('dashboard.invoicing.paid_month') }}</p>
                    <p class="mt-1 text-2xl font-semibold tracking-tight text-emerald-600">
                        {{ invoicing.paid_month?.formatted }}
                    </p>
                    <p class="mt-0.5 text-xs text-slate-400">
                        {{ tc('dashboard.invoicing.documents', invoicing.paid_month?.count ?? 0) }}
                    </p>
                </div>

                <Link href="/invoices" class="card p-4 transition hover:border-slate-300 hover:shadow-md">
                    <p class="text-xs text-slate-500">{{ t('dashboard.invoicing.outstanding') }}</p>
                    <p class="mt-1 text-2xl font-semibold tracking-tight text-slate-900">
                        {{ invoicing.outstanding?.formatted }}
                    </p>
                    <p class="mt-0.5 text-xs text-slate-400">
                        {{ tc('dashboard.invoicing.documents', invoicing.outstanding?.count ?? 0) }}
                    </p>
                </Link>
                ...
                >
                    <p class="text-xs text-slate-500">{{ t('dashboard.invoicing.overdue') }}</p>
                    <p
                    ...
                    <p class="mt-0.5 text-xs text-slate-400">
                        {{ tc('dashboard.invoicing.documents', invoicing.overdue?.count ?? 0) }}
                    </p>
                </Link>
            </div>
        </div>
        ...
        <div class="grid gap-6 lg:grid-cols-5">
            ...
            <CardSection
                ...
                :description="t('dashboard.forecast.description', { days: forecast.days, until: forecast.until })"
            >
                <p class="text-3xl font-semibold tracking-tight text-slate-900">{{ forecast.total }}</p>
                <p class="mt-1 text-xs text-slate-500">{{ t('dashboard.forecast.note') }}</p>

                <dl class="mt-5 space-y-3 border-t border-slate-100 pt-4 text-sm">
                    <div class="flex items-baseline justify-between gap-3">
                        <dt class="text-slate-600">
                            {{ t('dashboard.forecast.due') }}
                ...
                <template #footer>
                    <div class="flex flex-wrap items-center gap-x-6 gap-y-1 text-xs text-slate-500">
                        <span v-if="invoicing.avg_days_to_pay !== null">
                            {{ t('dashboard.forecast.avg_days_to_pay') }}
                            <strong class="text-slate-700">
                                {{ tc('dashboard.forecast.days', invoicing.avg_days_to_pay ?? 0) }}
                            </strong>
                        </span>
                        <span v-if="invoicing.drafts">
                            {{ t('dashboard.forecast.drafts') }}
                ...
                <div v-if="hasHistory">
                        ...
                        <div v-for="month in months" :key="month.key" class="flex h-full flex-1 flex-col justify-end">
                            <div class="flex h-full items-end justify-center gap-1">
                                <div
                                    ...
                                    :title="t('dashboard.history.bar_invoiced', { amount: money(month.invoiced_cents) })"
                                ></div>
                                <div
                                    class="w-1/2 rounded-t-md bg-emerald-500/80 transition-all"
                                    :style="{ height: height(month.paid_cents) }"
                                    :title="t('dashboard.history.bar_paid', { amount: money(month.paid_cents) })"
                                ></div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mt-4 flex items-center gap-4 text-xs text-slate-500">
                        <span class="flex items-center gap-1.5">
                            <span class="h-2.5 w-2.5 rounded-sm bg-brand-500/80"></span>
                            {{ t('dashboard.history.invoiced') }}
                        </span>
                        <span class="flex items-center gap-1.5">
                            <span class="h-2.5 w-2.5 rounded-sm bg-emerald-500/80"></span>
                            {{ t('dashboard.history.paid') }}
                        </span>
                    </div>
                </div>
            ...
            <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                ...
                >
                    ...
                    <p class="mt-4 text-sm text-slate-500">
                        ...
                        {{ tc('dashboard.products.organizations', product.organizations_count ?? 0) }}
                    </p>
                </Link>
        ...
        <div class="grid gap-6 lg:grid-cols-2">
                    ...
                    <li v-for="(row, i) in attention" :key="i" class="flex items-center justify-between gap-3 py-3">
                        <div class="min-w-0">
                            ...
                            <p class="text-xs text-slate-500">
                                {{ row.product }}
                                <span v-if="row.deadline"> · {{ t('dashboard.attention.deadline', { date: row.deadline }) }}</span>
                            </p>
                        </div>
            ...
            <CardSection
                icon="shield"
                tone="emerald"
                :title="t('dashboard.near_limit.title')"
                :description="t('dashboard.near_limit.description')"
            >
                <ul class="divide-y divide-slate-100">
